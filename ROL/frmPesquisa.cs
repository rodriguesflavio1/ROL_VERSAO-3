...
namespace ROL
{
    public partial class frmPesquisa : Form
    {
        ...
        EntidadeVersao entidadeVersao = new EntidadeVersao();
        ...
        private bool ConsultaDadosHMTeste()
        {

            entidade.NomeTabela = "HM";

            List<EntidadesRol> list = consulta.ResgataDadosHM(this.txtConsulta.Text, entidade.NomeTabela, entidade.NomeCampo, entidadeVersao.Banco);
        ...
        private bool ConsultaDadosSadt()
        {
            ...
            List<EntidadesRol> list = consulta.ResgataDadosSadt(this.txtConsulta.Text, entidade.NomeTabela, entidade.NomeCampo, entidadeVersao.Banco);
            if (list.Count > 0)
            {
        ...
        private void ConsultarVersao()
        {
            ...
            entidadeVersao.BancoDefault = @"\\10.10.0.25\ROL\bd.mdb";
            DataTable listaVersao = consulta.ResgataDadosVersao(entidadeVersao.BancoDefault);
            if (listaVersao.Rows.Count > 0)
            {                
                this.cbVersao.DataSource = listaVersao;
                this.cbVersao.DisplayMember = "versaoant";
                this.cbVersao.ValueMember = "codigo";
                //this.cbVersao.Text = "Selecione";
        ...
        private void ConsultaAba()
        {
            ...
            List<EntidadesRol> listaAba = consulta.ResgataDadosAba(this.txtConsulta.Text, entidade.TabelaAba, entidade.NomeCampo, entidadeVersao.Banco);
            if (listaAba.Count > 0)
            {
        ...
        private void ConsultaDut()
        {
            ...
            List<EntidadesRol> listaDut = consulta.ResgataDadosDut(this.txtConsulta.Text, entidade.NomeTabela, entidade.NomeCampo, entidadeVersao.Banco);
            if (listaDut.Count > 0)
            {
        ...
        private void ConsultarDocumentoDUT()
        {
            try
            {
                DataTable listaDocumento = consulta.ResgatarDocumentoDut(this.txtConsulta.Text, "arquivodut", "cod_servico", entidadeVersao.Banco);
        ...
        private void AbrirDocumento()
        {
            try
            {
                byte[] listaDocumento = consulta.AbrirDocumentoDut(Convert.ToString(dganexo.CurrentRow.Cells[0].Value), "arquivodut", "codigo", entidadeVersao.Banco);
        ...
        private void rbdAdminstrador_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                Login login = new Login(entidadeVersao.Banco);
                login.Show();
                this.Hide();
            }
        }
        ...
        private void dganexo_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                using (var com = AbrirConexao(entidadeVersao.Banco))
                {
                    com.Open();
        ...
        private void cbVersao_SelectionChangeCommitted(object sender, EventArgs e)
        {
            CaminhoBanco();
        }

        private void CaminhoBanco()
        {
            entidade.NomeCampo = "codigo";
            int id = Convert.ToInt32(cbVersao.SelectedValue.ToString());

            DataTable listaVersao = consulta.ResgataDadosBanco(id, entidade.NomeCampo, entidadeVersao.BancoDefault);

            if (listaVersao.Rows.Count > 0)
            {
                IEnumerable cons = from banco in listaVersao.AsEnumerable()
                                   select banco;
                foreach (DataRow dr in cons)
                {
                    entidadeVersao.Banco = dr[3].ToString();
                }
            }
        }

        private void RetornarValoresPorteMedico()
        {
            entidade.NomeCampo = "codigo";
            int id = Convert.ToInt32(cbVersao.SelectedValue.ToString());
